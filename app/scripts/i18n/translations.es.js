'use strict';

define([], function() {
  return {
    i18nMyProfile: 'Mi perfil',
    i18nLogout: 'Cerrar sesión',
    i18nWebAbbreviation: 'SGA',
    i18nWebName: 'Sistema de Gestión Académica',
    i18nAdminsPanelSection: 'Administradores',
    i18nStudentsPanelSection: 'Alumnos',
    i18nCoursesPanelSection: 'Materias',
    i18nAddAdmin: 'Agregar administrador',
    i18nDisableInscriptions: 'Deshabilitar inscripciones',
    i18nDisableInscriptionsModalTitle: '¿Desea deshabilitar las inscripciones para los alumnos?',
    i18nEnableInscriptions: 'Habilitar inscripciones',
    i18nEnableInscriptionsModalTitle: '¿Desea habilitar las inscripciones para los alumnos?',
    i18nConfirm: 'Confirmar',
    i18nCancel: 'Cancelar',
    i18nAdminsIndex: 'Ver todos',
    i18nStudentsIndex: 'Ver todos',
    i18nAddStudent: 'Agregar alumno',
    i18nCoursesIndex: 'Ver todas',
    i18nAddCourse: 'Agregar materia',

    // Admin subSidebar
    i18nAdminShow: 'Ver perfil',
    i18nAdminEdit: 'Editar perfil',
    i18nAdminResetPassword: 'Resetear contraseña',
    i18nAdminEditPassword: 'Cambiar contraseña',

    // Student subSidebar
    i18nStudentShow: 'Ver perfil',
    i18nStudentEdit: 'Editar perfil',
    i18nStudentResetPassword: 'Resetear contraseña',
    i18nResetPasswordModalTitle: 'Resetear contraseña',
    i18nStudentEditPassword: 'Cambiar contraseña',
    i18nStudentCourses: 'Materias en progreso',
    i18nStudentGrades: 'Analítico de notas',
    i18nStudentInscriptions: 'Matriculaciones',
    i18nStudentFinals: 'Inscripción a finales',
    i18nStudentDelete: 'Eliminar',
    i18nUserDelete: 'Eliminar',

    // Course subSidebar
    i18nCourseShow: 'Información',
    i18nCourseEdit: 'Editar',
    i18nCourseStudents: 'Alumnos inscriptos',
    i18nCourseApproved: 'Historial de aprobados',
    i18nCourseAddCorrelative: 'Agregar correlativa',
    i18nCourseDelete: 'Eliminar',

    // Tables common headers
    i18nActions: 'Acciones',

    // User Update
    i18nRequiredFields: 'Campos Obligatorios',
    i18nRequiredIcon: '*',
    i18nMale: 'Masculino',
    i18nFemale: 'Femenino',
    i18nBirthdayPlaceholder: 'yyyy-mm-dd',
    i18nButtonSaveChanges: 'Guardar Cambios',
    i18nButtonCancel: 'Cancelar',

    // User Show
    i18nStudents: 'Alumnos',
    i18nProfile: 'Perfil',
    i18nDni: 'DNI',
    i18nDocket: 'Legajo',
    i18nFirstName: 'Nombre',
    i18nLastName: 'Apellido',
    i18nBirthday: 'Fecha de Nacimiento',
    i18nEmail: 'Email',
    i18nGenre: 'Género',
    i18nTelephone: 'Teléfono',
    i18nCountry: 'País',
    i18nCity: 'Ciudad',
    i18nNeighborhood: 'Localidad',
    i18nStreet: 'Calle',
    i18nNumber: 'Altura',
    i18nFloor: 'Piso Nro.',
    i18nDoor: 'Dpto.',
    i18nZipCode: 'Cód. Postal',

    // Course Show
    i18nCourses: 'Materias',
    i18nInformation: 'Información',
    i18nCourseId: 'Código',
    i18nCourseName: 'Nombre',
    i18nCredits: 'Créditos',
    i18nSemester: 'Cuatrimestre',
    i18nCorrelatives: 'Correlativas',
    i18nDeleteCorrelative: 'Eliminar correlatividad',
    i18nNoCorrelativesFound: 'Esta materia no tiene correlativas',
    i18nFinalExams: 'Exámenes Finales',
    i18nVacancy: 'Cupos',
    i18nState: 'Estado',
    i18nFinalExamDate: 'Fecha del examen',
    i18nNoFinalExamsFound: 'No se encontraron fechas de examen final',
    i18nInscriptionOpen: 'Abierto',
    i18nInscriptionClosed: 'Cerrado',

    // Courses Students Index
    i18nQualify: 'Calificar',

    // Search filters
    i18nSearchButton: 'Buscar',
    i18nResetButton: 'Resetear',
    i18nNoStudentsFound: 'No se encontraron alumnos disponibles',
    i18nNoCoursesFound: 'No se encontraron materias disponibles',

    // Form errors
    i18nRequiredField: 'Este campo es requerido',
    i18nLengthPrefix: 'Este campo debe tener entre',
    i18nLengthAnd: 'y',
    i18nLengthSuffix: 'caracteres.'
  };
});
