...
public class CourseServiceImpl implements CourseService {
  ...
  public boolean deleteCourse(final String courseId) {
    ...
    if(courseDao.gradeExists(courseId)){
	  ...
	  final List<FinalInscription> courseFinalInscriptions = courseDao.getCourseFinalInscriptions(courseId);

	  if (courseFinalInscriptions != null && !courseFinalInscriptions.isEmpty()) {
		  return false;
    }
  ...
  public boolean addCorrelative(final String courseId, final String correlativeId) {
    ...
    if (course == null || correlativeCourse == null){
      return false;
    }

    final List<Course> available = getAvailableAddCorrelatives(courseId, null);
    if(!available.contains(correlativeCourse)){
      return false;
    }
  ...
  public List<Course> getAvailableAddCorrelatives(final String courseId, final CourseFilter courseFilter) {
	...
	@Transactional
	@Override
	public Map<Course, Boolean> getAvailableAddCorrelativesMap(final Course course) {
		final List<Course> courses = getByFilter(null);
		final List<Course> correlatives = getCorrelativeCourses(course.getCourseId());
		final Map<Course, Boolean> correlativesMap = new HashMap<>();
		...
		for (Course correlative : courses) {

			//Check correlativity loop && semester
			if (course.getSemester() <= correlative.getSemester() || courseDao.checkCorrelativityLoop(course.getCourseId(), correlative.getCourseId())) {
				correlativesMap.put(correlative, false);
			} else {
				correlativesMap.put(correlative, true);
			}
		}

		return correlativesMap;
	}
